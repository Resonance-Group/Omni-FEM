...
CEntry::CEntry()
{
	next = NULL;	// Sets the pointer for the next row to NULL
	x = 0;			// Sets the value stored in entry to 0
	c = 0;			// Sets column that the entry lives in to 0
}
...
///<summary> This function initilizes all of the arays that are used </summary>
///<returns> Returns a 1 when process is complete </returns>
int CBigLinProb::Create(int d, int bw)
{
	int i;

	bdw = bw;
	b = (double *)calloc(d, sizeof(double));		// Creating RHS of linear equation with d number of elements
	V = (double *)calloc(d, sizeof(double));		// Creating the solution vector
	P = (double *)calloc(d, sizeof(double));		// Creating the search directory
	R = (double *)calloc(d, sizeof(double));		// Creating the residual matrix
	U = (double *)calloc(d, sizeof(double));
	Z = (double *)calloc(d, sizeof(double));
	Q = (BOOL *)calloc(d, sizeof(BOOL));
	M = (CEntry **)calloc(d, sizeof(CEntry *));
	n = d;											// Sets the dimension of the matrix equal t d
