...
///<summary> Takes an input and squares the number </summary>
///<param=s> The number that is to be squared </param>
///<returns> Returns that square of x </returns>
double sq(double x)
{ 
	return x * x; 
}
...
///<summary>
/// This is the solver for the electrostatics simulation 
///</summary>
///<returns> Returns true if the function completes successfully </returns>
...
///<summary> Writes the results of the simulation is a file </summary>
...
BOOL CFemmeDocCore::WriteResults(CBigLinProb &L)
{
	...
	fprintf(fp, "%i\n", NumNodes);// First, print the total number of nodes in the system

	for (i = 0; i < NumNodes; i++)
		/* Prints out the mesh information (x, y points) along with the solution. Not sure what Q is */
		fprintf(fp, "%.17g	%.17g	%.17g	%i\n", meshnode[i].x / cf, meshnode[i].y / cf, L.V[i], L.Q[i]);

	fprintf(fp, "%i\n", NumEls);

	for (i = 0; i < NumEls; i++)
		/* With each triangle element, there is a data that describes the location of the 3 corner points (described by the p array) and the block label (described by the lbl)*/
