...
/* This file contains the code for the UI view */
...
///<summary> Begin the definition of the message map </summary>
///<param=CFemmView> Specifies the name of the class whose message map this is which in this case is the CFEMMiew</param>
///<param=CView> Specifies the name of the base class of CFemmView </param>
BEGIN_MESSAGE_MAP(CFemmeView, CView)
	//{{AFX_MSG_MAP(CFemmeView)
	ON_COMMAND(ID_NODE_OP, OnNodeOp)
	/* The formatting of each command is as follows:
		id - The command ID.
	*/
...
/* This function is called everytime that the mouse moves over the UI */
void CFemmeView::OnMouseMove(UINT nFlags, CPoint point) 
{
	static CPoint previousPoint;
	...
	{
		if ((currentPoint.x - previousPoint.x) > difference)
		{
			OnKeyDown(VK_LEFT, 1, 331);
			previousPoint = currentPoint;
		}
		else if ((currentPoint.x - previousPoint.x) < -(difference))
		{
			OnKeyDown(VK_RIGHT, 1, 333);
			previousPoint = currentPoint;
		}
		else if ((currentPoint.y - previousPoint.y) > difference)
		{
			OnKeyDown(VK_UP, 1, 328);
			previousPoint = currentPoint;
		}
		else if ((currentPoint.y - previousPoint.y) < -(difference))
		{
			OnKeyDown(VK_DOWN, 1, 336);
			previousPoint = currentPoint;
		}
	}
	else
...
/* This code is called everytime that the zoom in button is clicked */
...
BOOL CFemmeView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (zDelta < 0)
		OnZoomOut();
	else
...
/* This code is called everytime that the zoom out button is clicked */
...
///<summary> This code is called everytime a key on the keyboard is pressed down </summary>
void CFemmeView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if (nChar == VK_F3)
	{
		...
		if (GetBoundingBox(x, y) != FALSE)
			DefaultMeshSize = pow((double)(abs(x[1] - x[0] + I*(y[1] - y[0])) / BoundingBoxFraction), (double)2);
		else DefaultMeshSize = 0;
		for (i = 0; i < pDoc->blocklist.GetSize(); i++)
		{
			if (pDoc->blocklist[i].MaxArea>0) pDoc->blocklist[i].MaxArea /= 4.;
			else if (theApp.d_SmartMesh) pDoc->blocklist[i].MaxArea = DefaultMeshSize / 4.;
		}
		InvalidateRect(NULL);
	}
	...
	if (nChar==VK_F4)
	{
		...
		if ((GetBoundingBox(x,y)!=FALSE) && (theApp.d_SmartMesh))
			DefaultMeshSize=pow((double)(abs(x[1] - x[0] + I*(y[1] - y[0])) / BoundingBoxFraction), (double)2);
		else DefaultMeshSize=0;
...
///<summary> This function is called everytime the user left clicks the mouse suring the pre-processor screen. This is the down click </summary>
...
///<summary> This function is called everytime the user left clicks the mouse suring the pre-processor screen. This is the up click </summary>
...
///<summary> This function is called everytime the user right clicks the mouse during the pre-processor screen. This is the down click </summary>
...
///<summary> This function is called everytime the user double left clicks the mouse during the pre-processor screen.</summary>
...
///<summary> This function is called everytime the user double right clicks the mouse during the pre-processor screen.</summary>
