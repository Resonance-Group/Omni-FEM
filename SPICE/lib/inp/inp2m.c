...
/*
	Parses a SPICE2 format MOSFET card.
	Due to sifferences between SPICE2 and SPICE3 this routine is relatively ugly, first
	having to find the model the device is an instance of to determine the 
	type of MOSFET it is instantiating, a level 1, 2, 3, or BSIM device.
	If no model can be found by the given name, a default level 1 model is created 
	with that name and given no parameters
*/
...
void INP2M(GENERIC *ckt, INPtables *tab, card *current)
{
