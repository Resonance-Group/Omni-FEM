...
/*
	Fetches an expression from the given input line and breaks it into a parse tree.
	The trees necessary to ealuate that partial derivatives of the original tree with respect to each of the input
	variables will also be generated. 
*/
///<param name = "**line"> Line to parse </param>
///<param name = "**pt"> Parse tree to return </param>
///<param name = "*ckt"> THe current circuit </param>
///<param name = "*tab"> The symbol tables </param>
void INPgetTree(char **line, INPparseTree **pt, GENERIC *ckt, INPtables *tab)
{
	...
	(*pt)->derivs = (INPparseNode **)MALLOC(numvalues * sizeof(INPparseNode *));
 ...
 *** trees.
	...
	Differentiates an epression symbolically.
	The INPparseNOde returned contains a parse tree represents the 
	deivative of the INPparseNode given as an argument with respect to
	the variable given as varnum.
	This function is local to the INPgeTree routine and can not be used elsewhere.
 */
...
/*
	Generates a parse tree node corresponding to a constant with the given value.
	This function is local to the INPgetTree routine and can not be used elsewhere.
*/
...
/*
	Generates a parse tree for a binary operator connecting 2 other parse trees.
	An attempt is made to generate the simplest arse tree, so special cases for binary operators joining constants, multiplication
	by zero or one, division by one, zero divide by anything, addition or subtraction of zero,
	unary subtraction, and raising to the 0th or 1st power are all handled correctly.
	This function is local to the INPgetTree routine and be not be used elsewhere
*/
...
/*
	Generates a parse tree for a function call operation.
	Simplifications are performed if possible, so functions of a constant will be replaced by their actual constant value.
	This fucntion is local to the INPgetTree routine and can not be used elsewhere
*/
///<param name = "type"> Type of function </param>
///<param name = "*arg"> Argument to function </param>
...
/*
	Used the check the validity of a parse tree.
	There are some ambiguities of teh syntax used in our
	expressions which can not be resolved until the surrounding context has been examined.
	Such problem tokens are put in the tree unparsed and either handled later when appropriate or generate
	an error when this routine checks to be sure they are gone.
	This function is local to the INPgetTree routine
*/
///<param name = "*p"> Parse tree to check </param>
...
/*
	Actually parse an expression into a parse tree.
	This is the internal routine that does all the hard for INPgetTree.
	This funciton is local to teh INPgetTree routine.
*/
///<param name = "**line"> Line to parse </param>
...
/*
	Convert the given element into a parse tree node.
	Elem can be a string, number, or node.
	This fucntion is local to INPgetTree.
*/
///<param name = "*elem"> Element to convert </param>
...
/*
	Generates a parse tree node for a binary operator which combines the given two subtress
	as its left and right arguements.
	This function is local to INPgetTree.
*/
///<param name = "opnum"> Operator number </param>
///<param name = "arg1"> Left argument parse tree </param>
///<param name = "arg2"> Right argument parse tree </param>
...
/*
	COnverts a function call into the necessary parse tree node.
	SInce the oldspice notations V(nodename) and I(sourcename) are supported,
	mkfnode must detect these situations and convert
	them to the proper reference to a node voltage or device current instead of a function call.
	This function is local to the INPgetTree routine.
*/
...
/* Number node. */
/*
	Generates a parse node for a constant with a given value.
	This function is local to the INPgetTree routine
*/
///<param name = "number"> Constant value fo node </param>
...
/*
	Generates a parse tree node for a character string which may not be fully parsable yet.
	Cases for special signal names lik 'time' are handled here as well as known constants like pi.
	Unknown names are left alone with a placeholder type so that they can be picked up later as arguments to 
	the SPICE2 V(node) or I(source) notation.
	This function is local to the INPgetTree routine.
*/
///<param name = "*string"> The unkown string </param>
...
/*
	A lexical analyzer used for expression parsing.
	The tokens needed when parsing an expression are different from those used when parsing most of the rest of the SPICE input format.
	This function handles the different tokenizing needed for expressions.
	This function is local to the INPgetTree.
*/
...
/*
	A debugging routine used for printing an entire parse tree family, including all of
	the derivative tress.
	The string str is printed as a label on the top level tree.
	This function is local to teh INPgetTree routine.
*/
///<param name = "*str"> Tree name </param>
///<param name = "*ptree"> Tree to print </param>
...
/*
	A debugging function used to print a subtree of a parse tree.
	This function is local to the INPgetTree routine
*/
///<param name = "*pt"> Parse sub-tree to print </param>
