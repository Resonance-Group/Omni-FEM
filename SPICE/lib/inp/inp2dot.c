...
/*
	This routine sttempts to parse all of the control cards in the SPICE2 format whcih 
	begin with a dot.
	Some of these cards are picked off by an earlier layer of nutmeg, but many reach
	this level, such as the analysis commands.
	All are trapped at this level to avoid ambiguity if nutmeg misses one or 
	this parser is used outside nutmeg.
*/
///<param name = "*ckt"> The current circuit </param>
///<param name = "*tab"> THe symbol table </param>
///<param name = "*current"> The line to finish parsing </param>
///<param name = "*task"> The task new analysis will be in </param>
int INP2dot(GENERIC *ckt, INPtables *tab, card *current, GENERIC *task, GENERIC *gnode)
{
	...
	else if ((strcmp(token, ".width") == 0) || strcmp(token, ".print") == 0 || strcmp(token, ".plot") == 0)
	{
		/* obsolete - ignore */
	...
	else if ((strcmp(token, ".tf") == 0))
	{
		...
		if (*name == 'v' && strlen(name) == 1)
		{
			...
			if (*line != /* match ( */ ')')
			{
				...
				ptemp.sValue = (char *)MALLOC(sizeof(char)*(5 + strlen(nname1) + strlen(nname2)));
	...
	else if ((strcmp(token, ".subckt") == 0) || (strcmp(token, ".ends") == 0))
	{
		/* not yet implemented - warn & ignore */
	...
	else if ((strcmp(token, ".options") == 0) || (strcmp(token, ".option") == 0) || (strcmp(token, ".opt") == 0)) 
	{
	...
	else if ((strcmp(token, ".sens2") == 0))
	{
		...
		IFC(newAnalysis, (ckt, which, "Sensitivity-2 Analysis", &foo, task))
			...
			{ /* read the entire line */
				...
				{
					/* find the parameter */
					if (0 == strcmp(token, ft_sim->analyses[which]->analysisParms[i].keyword))
					{
						...
						if (ft_sim->analyses[which]->analysisParms[i].dataType & IF_FLAG)
						{
							/* one of the keywords! */
							ptemp.iValue = 1;
							error = (*(ft_sim->setAnalysisParm))(ckt, foo, ft_sim->analyses[which]->analysisParms[i].id, &ptemp, (IFvalue*)NULL);
							if (error)
								current->error = INPerrCat(current->error, INPerror(error));
						}
						else
						{
							...
							error = (*(ft_sim->setAnalysisParm))(ckt, foo, ft_sim->analyses[which]->analysisParms[i].id, parm, (IFvalue*)NULL);
