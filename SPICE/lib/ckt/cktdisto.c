...
int CKTdistro(register CKTcircuit *ckt, int mode)
{
	...
	case D_RHSF2:
	{
		...
		if (vcode >= 0)
		{
			...
			for (model = (VSRCmodel *)ckt->CKThead[vcode]; model != NULL; model = model->VSRCnextModel)
			{
				for (here = model->VSRCinstances; here != NULL; here = here->VSRCnextInstance)
				{
					...
					if (here->VSRCdGiven)
					{
						...
						if (((here->VSRCdF1given) && (mode == D_RHSF1)) || ((here->VSRCdF2given) && (mode == D_RHSF2)))
						{
		...
		if (icode >= 0)
		{
			...
			for (model = (ISRCmodel *)ckt->CKThead[icode]; model != NULL; model = model->ISRCnextModel)
			{
				for (here = model->ISRCinstances; here != NULL; here = here->ISRCnextInstance)
				{
					...
					if (here->ISRCdGiven)
					{
						...
						if (((here->ISRCdF1given) && (mode == D_RHSF1)) || ((here->ISRCdF2given) && (mode == D_RHSF2)))
						{
