 ...
 */
/* Stopped here on 9/29/2015 File unfinished */
#include "spice.h"
#include <stdio.h>
#include "pzdefs.h"
#include "util.h"
...
CKTpzFindZeros(CKTcircuit *ckt, PZtrial **rootInfo, int *rootCount)
{
	...
	} while (High_Guess - Low_Guess < 1e40 && NZeros < Max_Zeros && NIter < NITER_LIM && Aberr_Num < 3 && High_Guess - Low_Guess < 1e35	/* XXX Should use mach const */ && (!neighborhood[0] || !neighborhood[2] || CKTpzTrapped || neighborhood[2]->s.real - neighborhood[0]->s.real < 1e22));
	/* XXX ZZZ */
	...
	*rootCount = NZeros;
	...
	if (Aberr_Num > 2)
	{
		sprintf(ebuf, "Pole-zero converging to numerical aberrations; giving up after %d trials", Seq_Num);
		(*(SPfrontEnd->IFerror))(ERR_WARNING, ebuf, 0);
	}

	if (NIter >= NITER_LIM)
	{
		sprintf(ebuf, "Pole-zero iteration limit reached; giving up after %d trials", Seq_Num);
		(*(SPfrontEnd->IFerror))(ERR_WARNING, ebuf, 0);
	}
...
int PZeval(int strat, PZtrial *set[], PZtrial **newTrialP)
{
	int		error;
	double	a, b, est, k, n;
	PZtrial	*new_trial;
	...
	case SYM2:
		...
		if (CKTpzTrapped == 1)
		{
			if (new_trial->s.real < set[0]->s.real || new_trial->s.real > set[1]->s.real)
			{
#ifdef PZDEBUG
				DEBUG(1) fprintf(stderr,
		...
		{
			if (new_trial->s.real < set[1]->s.real || new_trial->s.real > set[2]->s.real)
			{
#ifdef PZDEBUG
				DEBUG(1) fprintf(stderr,
					"FIXED UP BAD Strat: %s (%d) was (%.15g,%.15g)\n",
		...
		{
			if (new_trial->s.real <= set[0]->s.real || new_trial->s.real == set[1]->s.real && new_trial->s.imag == set[1]->s.imag || new_trial->s.real >= set[2]->s.real)
			{
#ifdef PZDEBUG
				DEBUG(1)
					fprintf(stderr,
...
CKTpzStrat (PZtrial **set)
{
	...
	else
	{
		...
		if (!CKTpzTrapped)
		{
			...
			if (a_mag + 10 < set[0]->mag_def && a_mag + 10 < set[1]->mag_def && b_mag + 10 < set[1]->mag_def && b_mag + 10 < set[2]->mag_def)
			{
				if (k1 > k2)
