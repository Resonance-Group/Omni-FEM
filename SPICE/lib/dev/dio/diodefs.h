...
typedef struct sDIOinstance 
{
    ...
    struct sDIOinstance *DIOnextInstance;   /* pointer to next instance of current model*/
    ...
    double *DIOposPosPrimePtr;				/* pointer to sparse matrix at (positive, positive prime) */
    double *DIOnegPosPrimePtr;				/* pointer to sparse matrix at (negative, positive prime) */
    double *DIOposPrimePosPtr;				/* pointer to sparse matrix at (positive prime, positive) */
    double *DIOposPrimeNegPtr;				/* pointer to sparse matrix at (positive prime, negative) */
    double *DIOposPosPtr;					/* pointer to sparse matrix at (positive, positive) */
    double *DIOnegNegPtr;					/* pointer to sparse matrix at (negative, negative) */
    double *DIOposPrimePosPrimePtr;			/* pointer to sparse matrix at (positive prime, positive prime) */

    double DIOcap;							/* stores the diode capacitance */

    double *DIOsens;						/* stores the perturbed values of geq and ceq in ac sensitivity analyis */

    int DIOsenParmNo ;						/* parameter # for sensitivity use. set equal to 0 if not a design parameter*/
    ...
    unsigned DIOsenPertFlag :1;				/* indictes whether the the parameter of the particular instance is to be perturbed */
    ...
    double DIOtDepCap;						/* temperature adjusted transition point in the curve matching (Fc * Vj ) */
...
#define DIOsensxp DIOstate+5    /* charge sensitivities and their derivatives. +6 for the derivatives - pointer to the beginning of the array */


/* per model data */

/* model structure for a diode */
typedef struct sDIOmodel 
{       
    int DIOmodType;								/* type index of this device type */
    struct sDIOmodel *DIOnextModel;				/* pointer to next possible model in linked list */
    DIOinstance * DIOinstances;					/* pointer to list of instances that have this model */
