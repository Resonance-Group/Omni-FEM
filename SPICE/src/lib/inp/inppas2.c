...
/*
	The second pass of the SPICE2 compatible input parse, this routine examines all of the SPICE input lines except
	for ".MODEL" cards.
	Each type of input line, as determined by teh first character of the line, is actually handled by a seperate routine to 
	keep this file from becoming too large.
	By convention, the routine hanling the individual device lines are named INP2x where x is the keyletter used by SPICE
	for the first letter of the device name.
	All of the control, or "dot" cards are handled by INP2dot.
*/
///<param name = "*ckt"> The circuit structure to parse into </param>
///<param name = "*deck"> The input deck to parse </param>
///<param name = "*tab"> The symbol table for this circuit </param>
///<param name = "*task"> The "task" to put analysis requests in </param>
void INPpas2(GENERIC *ckt, card *data, INPtables *tab, GENERIC *task)
{
	...
	if (error)
		data->error = INPerrCat(data->error, INPmkTemp("can't read internal ground node name!\n"));

	error = INPgndInsert(ckt, &gname, tab, &gnode);
	if (error && error != E_EXISTS)data->error = INPerrCat(data->error, INPmkTemp("can't insert internal ground node in symbol table!\n"));
