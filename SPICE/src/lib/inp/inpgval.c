...
/*
	Used to get a value of type type from the input line without haing to know anything 
	about type itself.
	INPgetValue performs all of the type specific operations, evaluating numbers, 
	inserting strings into the string table, and placing the final result into an IFvalue structure.
	Since the IFvalue structure which is returned is a static, the structure musst be used or its value copied out
	before any other calls are made to INPgetValue or any subroutine whcih calls it.
*/
///<param name = "*ckt"> The current circuit </param>
///<param name = "**line"> THe line to get the arguement from </param>
///<param name = "type"> THe type of argument to get </param>
