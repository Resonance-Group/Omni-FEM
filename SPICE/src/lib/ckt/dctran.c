...
/*
	The transient analysis is the largest and most complicated simulation currently supported
	by SPICE3.
	This analysis permits the time dependent behaiour of the circuit to be analyzed.
*/
...
/*
	This routine actually performs the transient analysis.
	The algorithms are adequately explained elsewhere and
	this routine contains relatively straightforward implementations of them.
	The overall code is moderately complicated since so many of the 
	algorithms come together and interact here, particulary the 
	stop and restart code which takes up much of the first third of the function
*/
///<param name = '*ck'> Circuit to operate on </param>
///<param name = 'restart'> Start over ot resume </param>
int DCtran(CKTcircuit *ckt, int restart)
{
    int i;
    double olddelta;
    double delta;
	...
	/* end LTRA code addition */
        ...
        *(ckt->CKTbreaks+1)=ckt->CKTfinalTime;
        ...
        (*(SPfrontEnd->IFnewUid))((GENERIC *)ckt,&timeUid,(IFuid)NULL, "time", UID_OTHER, (GENERIC **)NULL);

        error = (*(SPfrontEnd->OUTpBeginPlot))((GENERIC *)ckt, (GENERIC*)ckt->CKTcurJob, ckt->CKTcurJob->JOBname,timeUid,IF_REAL,numNames,nameList, IF_REAL,&(((TRANan*)ckt->CKTcurJob)->TRANplot));
        ...
        converged = CKTop(ckt, (ckt->CKTmode & MODEUIC)|MODETRANOP| MODEINITJCT, (ckt->CKTmode & MODEUIC)|MODETRANOP| MODEINITFLOAT, ckt->CKTdcMaxIter);
        ...
        bcopy((char *)ckt->CKTstate0,(char *)ckt->CKTstate1, ckt->CKTnumStates*sizeof(double));
