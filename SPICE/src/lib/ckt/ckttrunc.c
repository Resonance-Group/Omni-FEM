...
/*
	The main calling point for the truncation error calculation.
	CKTtrunc will successively call the truncation error functions for every device which has one and set timeStep to the 
	minimum timestep required by any of the devices to meet the truncation error requirement.
*/
///<param = *ckt> Circuit to operate on </param>
///<param = *timeStep> Returned new timestep </param>
int CKTtrunc(register CKTcircuit *ckt, double *timeStep)
{
#ifndef NEWTRUNC
	register int i;
	double timetemp;
	...
	*timeStep = MIN(2 * *timeStep, timetemp);
	...
	case TRAPEZOIDAL:
		...
		case 1:
			for (i = 1; i < size; i++)
			{
				tol = MAX(FABS(ckt->CKTrhs[i]), FABS(ckt->CKTpred[i]))*ckt->CKTlteReltol + ckt->CKTlteAbstol;
				node = node->next;
				if (node->type != 3)
					continue;
				diff = ckt->CKTrhs[i] - ckt->CKTpred[i];
#ifdef STEPDEBUG
				printf("%s: cor=%g, pred=%g ", node->name, ckt->CKTrhs[i], ckt->CKTpred[i]);
#endif
				if (diff != 0)
				{
...
#endif
				if (diff != 0) {
					tmp = ckt->CKTdeltaOld[0] * ckt->CKTtrtol * tol * 3 * (ckt->CKTdeltaOld[0] + ckt->CKTdeltaOld[1]) / diff;
					tmp = FABS(tmp);
					timetemp = MIN(timetemp, tmp);
#ifdef STEPDEBUG
					printf("tol = %g, diff = %g, h->%g\n", tol, diff, tmp);
