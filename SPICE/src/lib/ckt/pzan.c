...
/*
	Performs that actual pole zero analysis itself using the Muller's method.
	This analysis computes the poles and zeros of the AC small signal transfer function
*/
///<param name = "*ckt"> Circuit to operate on </param>
///<param name = "restart"> Start over or resume </param>
int PZan(CKTcircuit *ckt, int reset)
{
	...
	error = CKTop(ckt, (ckt->CKTmode & MODEUIC) | MODEDCOP | MODEINITJCT, (ckt->CKTmode & MODEUIC) | MODEDCOP | MODEINITFLOAT, ckt->CKTdcMaxIter);
	if (error)
		return(error);
	...
	if (ckt->CKTkeepOpInfo)
	{
		...
		error = (*(SPfrontEnd->OUTpBeginPlot))((GENERIC *)ckt, (GENERIC*)ckt->CKTcurJob, "Distortion Operating Point", (IFuid)NULL, IF_REAL, numNames, nameList, IF_REAL, &plot);
		if (error)
			return(error);
