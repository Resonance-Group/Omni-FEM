...
int CKTop(CKTcircuit *ckt, long firstMode, long continueMode, int iterlim)
{
	...
	if (converged != 0)
	{
		...
		{
			ckt->CKTmode = firstMode;
			(*(SPfrontEnd->IFerror))(ERR_INFO, "starting Gmin stepping", (IFuid *)NULL);
			...
			for (i = 0; i <= ckt->CKTnumGminSteps; i++)
			{
				...
				{
					ckt->CKTdiagGmin = 0;
					(*(SPfrontEnd->IFerror))(ERR_WARNING, "Gmin step failed", (IFuid *)NULL);
					break;
				}
				ckt->CKTdiagGmin /= 10;
				ckt->CKTmode = continueMode;
				(*(SPfrontEnd->IFerror))(ERR_INFO, "One successful Gmin step", (IFuid *)NULL);
			}
			ckt->CKTdiagGmin = 0;
			converged = NIiter(ckt, iterlim);
			if (converged == 0)
			{
				(*(SPfrontEnd->IFerror))(ERR_INFO, "Gmin stepping completed", (IFuid *)NULL);
				return(0);
			}
			(*(SPfrontEnd->IFerror))(ERR_WARNING, "Gmin stepping failed", (IFuid *)NULL);
	 ...
	 *
		...
		{
			ckt->CKTmode = firstMode;
			(*(SPfrontEnd->IFerror))(ERR_INFO, "starting source stepping", (IFuid *)NULL);
			for (i = 0; i <= ckt->CKTnumSrcSteps; i++)
			{
				...
				if (converged != 0)
				{
					...
					(*(SPfrontEnd->IFerror))(ERR_WARNING, "source stepping failed", (IFuid *)NULL);
					return(converged);
				}
				(*(SPfrontEnd->IFerror))(ERR_INFO, "One successful source step", (IFuid *)NULL);
			}
			(*(SPfrontEnd->IFerror))(ERR_INFO, "Source stepping completed", (IFuid *)NULL);
			ckt->CKTsrcFact = 1;
			return(0);
		}
		else {
...
/*
	Loops through the device specific convergence test routines until one of them signals 
	non-convergence by incrementing the member CKTnoncon in the CKT circuit structure or all of them
	have been called
*/
///<param = *ckt> Circuit to operate on <param>
int CKTconvTest(register CKTcircuit *ckt)
{
