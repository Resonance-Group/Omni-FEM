...
/*
	Is fairly straightforward except for the stepping fules which have to hadnle positive
	and negative steps, rounding error, and missing nesting levels.
	Although previous versions of SPICE have only supported two levels of nesting,
	this routine supports an arbitrary level of nesting should such a capability be desired in the 
	future and the input system modified to support it.
	The only other unusual feature of this code is the attempt made to use 
	MODINTPRED to predict a value in the transfer curve and the placement of the voltage differences
	in the array of time deltas to allow such predictions.
	These predictations frequently prouce much faster convergence although a standard DC soultion is performed if any difficulties
	are encountered
*/
///<param name = "*ckt"> Circuit to operate on </param>
///<param name = "restart"> Start over or resume </param>
int DCtrCurv(CKTcircuit *ckt, int restart)
{
