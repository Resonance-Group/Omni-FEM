...
int CKTnoise(CKTcircuit *ckt, int mode, int operation, Ndata *data)
{
	...
	for (i = 0; i < DEVmaxnum; i++)
	{
		...
		{
			error = (*((*DEVices[i]).DEVnoise))(mode, operation, ckt->CKThead[i], ckt, data, &outNdens);
			if (error)
				return (error);
		}
	}
	...
	case N_OPEN:
		...
		case N_DENS:
			...
			(*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]), (IFuid)NULL, "onoise_spectrum", UID_OTHER, (GENERIC **)NULL);

			data->namelist = (IFuid *)trealloc((char *)data->namelist, (data->numPlots + 1)*sizeof(IFuid));

			(*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]), (IFuid)NULL, "inoise_spectrum", UID_OTHER, (GENERIC **)NULL);

			/* we've added two more plots */

			data->outpVector = (double *)MALLOC(data->numPlots * sizeof(double));
			break;

		case INT_NOIZ:

			data->namelist = (IFuid *)trealloc((char *)data->namelist, (data->numPlots + 1)*sizeof(IFuid));
			(*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]), (IFuid)NULL, "onoise_total", UID_OTHER, (GENERIC **)NULL);

			data->namelist = (IFuid *)trealloc((char *)data->namelist, (data->numPlots + 1)*sizeof(IFuid));
			(*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]), (IFuid)NULL, "inoise_total", UID_OTHER, (GENERIC **)NULL);
			/* we've added two more plots */
	...
	case N_CALC:
		...
		case N_DENS:
			if ((((NOISEAN*)ckt->CKTcurJob)->NStpsSm == 0) || data->prtSummary)
			{
