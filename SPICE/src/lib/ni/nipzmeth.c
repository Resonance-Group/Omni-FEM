...
/*
	Provides an implementation of Muller's method of finding the roots of a complex polynomial
*/
///<param name = "*set"> No documentation </param>
///<param name = "*newtry> No documentation </param>
int NIpzMuller(PZtrial *set[3], PZtrial *newtry)
{
	SPcomplex	A, B, C, D, E;
	SPcomplex	h0, h1;
	SPcomplex	j0, j1;
	...
	while (FABS(A.real) > 1.0 || FABS(A.imag) > 1.0 || FABS(B.real) > 1.0 || FABS(B.imag) > 1.0 || FABS(C.real) > 1.0 || FABS(C.imag) > 1.0)
	{
		A.real /= 2.0;
		B.real /= 2.0;
		C.real /= 2.0;
