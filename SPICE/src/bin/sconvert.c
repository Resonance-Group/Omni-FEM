...
static struct plot *oldread(char *name)
{
    ...
    if (i % 8) 
	{    /* Data points are always 8 bytes... */
    ...
    for (v = pl->pl_dvecs; v; v = v->v_next) 
	{
        ...
        if (isreal(v)) 
		{
            v->v_realdata = (double *) tmalloc(sizeof (double)* np);
        }
		else 
		{
...
/* ARGSUSED */ 
int cp_evloop(char *s)
{ 
	return (0); 
}

/* ARGSUSED */
void cp_ccon(o) bool o; 
{

}

/* ARGSUSED */ 
char *if_errstring(int c) 
{
	return ("error"); 
}
...
/* VARARGS1 ARGSUSED */ 
void out_printf(char *fmt,int args) // Data type for args is currenlty unknown int is there temporerliary
{ 
