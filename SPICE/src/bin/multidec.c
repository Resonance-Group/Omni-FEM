...
main(int argc, char **argv)
{
	...
	err = spOrderAndFactor(othermatrix, rhs, THRSH, ABS_THRSH, DIAG_PIVOTING);
	...
	case spNO_MEMORY:
		...
		exit(1);
	case spSINGULAR:
		(void)spWhereSingular(othermatrix, &singular_row, &singular_col);
	...
	for (j = 1; j <= num; j++)
	{
		fprintf(stdout, "e%d %d %d %d 0 %0.12g\n", (j - 1)*num + 1, node, 2 * num + j, num + 1, matrix[j][1]);
		node++;
		for (i = 2; i < num; i++)
		{
			fprintf(stdout, "e%d %d %d %d 0 %0.12g\n", (j - 1)*num + i, node, node - 1, num + i, matrix[j][i]);
			node++;
		}

		fprintf(stdout, "e%d %d %d %d 0 %0.12g\n", j*num, j, node - 1, 2 * num, matrix[j][num]);
	}
